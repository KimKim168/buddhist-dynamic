...
class SlideController extends Controller
{
    public function index(Request $request)
    {
        ...
        $position = $request->query('position'); // Get the position from the query string

        $query = Slide::latest();

        if ($position) {
            $query->where('position', $position); // Filter by position if provided
        }

        $slides = $query->select('id', 'name', 'image', 'position')->get();

        return response()->json([
            'slides' => $slides,
            'imagePath' => config('APP_URL') . '/assets/images/slides',
            'imageCompressedPath' => config('APP_URL') . '/assets/images/slides/thumb',
        ]);
    }
